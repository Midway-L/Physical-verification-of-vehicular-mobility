...
class SimpleEKF:
    def __init__(self, dt, init_x, init_P):
        ...
        self.P = np.diag([init_P[0,0], init_P[1,1], 0.5])  # 初始不确定性

        # 初始协方差矩阵的动态调整
        self.reset_threshold = 5.0  # 位置误差阈值
        self.adaptive_factor = 1.0  # 自适应因子
        
        # 动态过程噪声
        self.Q_base = np.diag([0.5, 0.5, 0.1])  # 基础过程噪声
        self.Q = self.Q_base.copy()
        
        # 改进的测量噪声
        self.R_base = np.diag([0.1, 0.1, 0.2, 0.1])  # 基础测量噪声
    ...
    def predict(self):
        ...
        # 根据历史创新序列调整过程噪声
        if len(self.innovation_window) > 0:
            innovation_std = np.std(self.innovation_window, axis=0)
            # 根据创新序列的标准差动态调整过程噪声
            self.Q = self.Q_base * (1 + np.mean(innovation_std[:2]))
    ...
    def update(self, z):
        ...
        z_pred = h(self.x)
        innovation = z - z_pred
        innovation[3] = angle_difference(z[3], z_pred[3])  # 使用角度差异函数

        # 保存创新序列
        self.last_innovation = innovation
        self.innovation_window.append(innovation[:2])  # 只保存位置创新
        ...
        # 动态调整测量噪声
        R_scaled = self.R_base.copy()
        if innovation_cov > self.reset_threshold:
            # 如果创新太大，增加状态不确定性
            self.P *= 2.0
            R_scaled *= 0.5  # 减小测量噪声以更快跟踪
        ...
        # 使用自适应因子调整状态更新
        innovation_magnitude = np.sqrt(np.sum(innovation[:2]**2))
        self.adaptive_factor = 1.0 + np.tanh(innovation_magnitude / 5.0)
        dx *= self.adaptive_factor
        ...
        I = np.eye(3)
        self.P = (I - K @ H) @ self.P @ (I - K @ H).T + K @ R_scaled @ K.T
        self.ensure_positive_definite()
...
def process_lidar_data(lidar_points, ego_x, ego_y, ego_heading, timestamp):
    ...
    clustering = DBSCAN(eps=1, min_samples=5).fit(np.array(valid_points))
...
def analyze_lidar_error(cluster_points, rx, ry, relative_distance, relative_angle):
    try:
        # 只使用x和y坐标计算协方差
        covariance = np.cov(cluster_points[:, :2].T)
        eigenvalues, _ = np.linalg.eig(covariance)
        point_spread = np.sqrt(np.sum(eigenvalues))
    except np.linalg.LinAlgError:
        # 如果出现线性代数错误，使用一个默认值
        point_spread = 1.0
        print("Linear algebra error occurred")

    # 2. 距离相关误差分析 (只使用x和y坐标)
    distances = np.sqrt(np.sum(cluster_points[:, :2]**2, axis=1))
    ...
    # 6. 计算z轴方向的分布
    z_spread = np.std(cluster_points[:, 2])
...
def save_ekf_model(ekf, filename='ekf_model.pkl'):
    model_data = {
        'x': ekf.x,
        'P': ekf.P,
        'Q': ekf.Q,
...
def plot_rx_ry(results):
    ...
    ax1.plot(timestamps, true_rx, label='True rx', color='green')
    ax1.plot(timestamps, estimated_rx, label='Estimated rx', color='blue')
    ax1.set_title('rx Comparison')
    ax1.set_xlabel('Timestamp')
    ax1.set_ylabel('rx (m)')
    ax1.legend()
    ax1.grid(True)

    ax2.plot(timestamps, true_ry, label='True ry', color='green')
    ax2.plot(timestamps, estimated_ry, label='Estimated ry', color='blue')
    ax2.set_title('ry Comparison')
...
def main():
    ...
    plot_rx_ry(results)
    save_ekf_model(ekf, 'my_ekf_model.pkl')
