...
ground_truth_t, ground_truth_d = read_json_file('cars_distance.json') # carla distance as ground truth
lidar_lead_t, lidar_lead_d = read_lidar_data('processed_data_197.json') # processed lead vehicle data
lidar_follow_t, lidar_follow_d = read_lidar_data('processed_data_201.json') # processed follow vehicle data
rssi_t, rssi_d = read_json_file('improved_rssi_distance.json') # improved rssi distance
sumo_t, sumo_d, sumo_rs, sumo_ls, sumo_fs = read_sumo_data('sumo_ground.json') # sumo distance and speed
veins_t, veins_d = read_veins_data('veins_distance.json') # veins distance

# Figure 1: Ground Truth vs LiDAR vs RSSI
fig1, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 16))
data_list = [(lidar_lead_t, lidar_lead_d), (lidar_follow_t, lidar_follow_d), (rssi_t, rssi_d)]
...
plt.savefig('ground_truth_lidar_rssi_comparison.png', dpi=300, bbox_inches='tight')
...
plt.tight_layout(h_pad=3.0)
plt.savefig('ground_truth_sumo_veins_comparison.png', dpi=300, bbox_inches='tight')
