...
# get data in processed vehicle file, used for time filter here
processed_data = []
...
with open('filtered_lidar_data_vehicle_id.json', 'w') as f:
    for item in new_data:
        json.dump(item, f)
        f.write('\n')

print(f"Origin frame number: {len(lidar_data)}")
print(f"processed_data frames as standard: {len(processed_data)}")
print(f"filter frames: {len(filtered_lidar_data)}")
print(f"new frames: {len(new_data)}")
