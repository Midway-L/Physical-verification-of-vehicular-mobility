...
def calculate_angle(x: float, y: float) -> float:
    ...
    参数:
    x (float): 相对于车辆的x坐标（右侧为正）
    y (float): 相对于车辆的y坐标（前方为正）
    
    返回:
    float: 角度（度），以车辆前方为0度，顺时针为正，范围 [0, 360)
    """
    if abs(x) < 1e-6 and abs(y) < 1e-6:
        return 0.0  # 如果点非常接近原点，返回0度
    
    # 计算原始角度
    raw_angle = math.degrees(math.atan2(y, x))
    
    # 调整角度：
    # 1. 首先旋转90度，使0度对准y轴正方向（车辆前方）
    # 2. 然后取反，使角度变为顺时针方向
    # 3. 最后确保角度在[0, 360)范围内
    adjusted_angle = (90 - raw_angle) % 360
...
def process_data(vehicle_id: str, input_dir: str, output_dir: str):
    ...
    std_file = os.path.join(input_dir, f"std_vehicle_290.json")
...
def main():
    ...
    parser.add_argument("vehicle_id", help="Vehicle ID")
