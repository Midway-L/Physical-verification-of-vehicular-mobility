...
def process_data(vehicle_id: str, input_dir: str, output_dir: str):
    ...
    std_file = os.path.join(input_dir, f"std_vehicle_151.json")
    ...
    for sensor, file in sensor_files.items():
        ...
        if sensor == 'lidar':
            ...
            processed_data = {ts: {'acc_x': data[ts]['accelerometer'][0], 'acc_y': data[ts]['accelerometer'][1],
                                   'gyro_x': data[ts]['gyroscope'][0], 'gyro_y': data[ts]['gyroscope'][1],'compass': data[ts]['compass']} for ts in std_timestamps if ts in data}
