...
def read_sumo_data(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            ...
            timestamps.append(data['new_timestamp'])
            distances.append(data['distance'])
            speeds.append(data['relative_speed'])
    return timestamps, distances, speeds
...
lidar_197_t, lidar_197_d = read_lidar_data('processed_data_197.json')
lidar_201_t, lidar_201_d = read_lidar_data('processed_data_201.json')
rssi_t, rssi_d = read_json_file('rssi_distance.json')
sumo_t, sumo_d, sumo_s = read_sumo_data('sumo_ground.json')
...
labels = ['Vehicle 197 LiDAR', 'Vehicle 201 LiDAR', 'RSSI']
...
ax2.plot(sumo_t, sumo_s, color='red')
ax2.set_xlabel('Time (s)')
ax2.set_ylabel('Relative Speed (m/s)')
ax2.set_title('SUMO Relative Speed vs Time', pad=20)
ax2.grid(True)

plt.tight_layout(h_pad=3.0)
plt.savefig('sumo_distance_and_speed.png', dpi=300, bbox_inches='tight')
