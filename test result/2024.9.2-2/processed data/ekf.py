...
# 读取数据
sumo_ground = read_json_file('sumo_ground.json')
processed_data_201 = read_json_file('processed_data_201.json')
rssi_distance = read_json_file('rssi_distance.json')
processed_data_197 = read_json_file('processed_data_197.json')
...
def main():
    # 初始化参数
    initial_x = processed_data_197[0]['x'] - processed_data_201[0]['x']
    initial_y = processed_data_197[0]['y'] - processed_data_201[0]['y']
    ...
    def run_filter(filter_obj, is_ekf=True):
        ...
        for i in range(len(sumo_ground)):
            ...
            measurement = np.array([
                ...
                processed_data_201[i]['lidar_avg_x'],
                processed_data_201[i]['lidar_avg_y']
            ])
            ...
            true_x = processed_data_197[i]['x'] - processed_data_201[i]['x']
            true_y = processed_data_197[i]['y'] - processed_data_201[i]['y']
