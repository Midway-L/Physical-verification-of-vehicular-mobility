...
def global_to_local(global_x, global_y, ego_x, ego_y, ego_compass, target_compass):
    ...
    # 计算距离
    distance = np.sqrt(dx**2 + dy**2) - 7.4
...
def main():
    # 加载数据
    data_lead = load_json_file('processed_data_282.json')
    data_follow = load_json_file('processed_data_278.json')
    lidar_data = load_json_file('filtered_lidar_data_282.json')
