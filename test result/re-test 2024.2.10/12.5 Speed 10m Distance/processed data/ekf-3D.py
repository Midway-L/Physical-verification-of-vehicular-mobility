...
def process_lidar_data(lidar_points, ego_x, ego_y, ego_heading, timestamp):
    ...
    clustering = DBSCAN(eps=0.5, min_samples=10).fit(np.array(valid_points))
    ...
    relative_distance = np.sqrt(rx**2 + ry**2) + 2.4
...
def main():
    # 加载数据
    data_151 = load_json_file('processed_data_421.json')
    data_147 = load_json_file('processed_data_417.json')
    lidar_data = load_json_file('filtered_lidar_data_421.json')
