...
def process_lidar_data(lidar_points, ego_x, ego_y, ego_heading, timestamp):
    ...
    if len(unique_labels) == 1 and unique_labels[0] == -1:  # take them as noise
        return None
...
def random_offset_attack(true_x, true_y, max_offset, timestamp, last_change_time, current_offset):
    if timestamp - last_change_time >= 10: 
...
def simulate_attack(attack_type, true_rx, true_ry, current_time, lidar_measurement, last_change_time, current_offset, last_jump_time, current_jump, attack_start_time):
    if attack_type == 'constant':
        return (true_rx + 5, true_ry + 5), last_change_time, current_offset, last_jump_time, current_jump
    elif attack_type == 'random':
        if current_time - last_change_time >= 5:
            magnitude_x = random.uniform(3, 7)
            magnitude_y = random.uniform(3, 7)
            sign_x = random.choice([-1, 1])
            sign_y = random.choice([-1, 1])
            current_offset = (sign_x * magnitude_x, sign_y * magnitude_y)
        ...
        if current_time - last_jump_time >= 5:
            ...
            sign_x = random.choice([-1, 1])
            sign_y = random.choice([-1, 1])
            current_jump = (sign_x * magnitude_x, sign_y * magnitude_y)
...
def main():
    ...
    # Load data
    data_follow = load_json_file('processed_data_follow_id.json')
    data_lead = load_json_file('processed_data_lead_id.json')
    lidar_data = load_json_file('filtered_lidar_data_follow_id.json')
    ...
    for attack_type in attack_types:
        ...
        for timestamp in sorted(data_follow.keys()):
            ...
            if attack_start_time is None:
                attack_start_time = current_time + 30  # start after sim 30s
            if attack_end_time is None:
                attack_end_time = attack_start_time + 70  # last for 70s
