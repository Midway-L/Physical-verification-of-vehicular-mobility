    ...
    def plot_results(self):
        ...
        for phase, color in phase_colors.items():
            if phase in self.phase_distances:
                ...
                ax1.axhline(y=target_dist, color=color, linestyle='--', alpha=0.3, label=f'{phase} Target ({target_dist}m)')
        ...
        phase_changes = self.plot_data['phase_changes'] + [(self.plot_data['timestamps'][-1], self.plot_data['phases'][-1])]

        for i in range(len(phase_changes)):
            time, phase = phase_changes[i]
            ...
            ax5.text(mid_point, 0.5, current_phase, ha='center', va='center', rotation=0)
        
            # 在其他图表中添加阶段变化线
            if current_phase != 'INACTIVE' and i < len(phase_changes) - 1:
