...
class SimulationSynchronization(object):
    def __init__(self, sumo_simulation, carla_simulation, tls_manager='none', sync_vehicle_color=False, sync_vehicle_lights=False):
        ...
        #self.challenge_duration = 60  # seconds in total challenge, add all phase durations
        self.max_protocol_cycles = 8  # 总共需要循环8次
        self.current_cycle = 0  # 当前是第几次循环
        self.protocol_completed = False  # 仅在所有循环完成后才为True
    ...
    def update_challenge_metrics(self, distance):
        ...
        if self.challenge_active:
            ...
            # 更新总体统计
            if not hasattr(self, 'overall_min_distance'):
                self.overall_min_distance = float('inf')
            if not hasattr(self, 'overall_max_distance'):
                self.overall_max_distance = 0
            
            self.overall_min_distance = min(self.overall_min_distance, distance)
            self.overall_max_distance = max(self.overall_max_distance, distance)
    ...
    def activate_challenge(self, distance, speed):
        ...
        # 初始化第一个循环的指标
        first_cycle_metrics = self.cycle_template.copy()
        first_cycle_metrics.update({
        'cycle_number': 0,
        ...
        'current_cycle': 1,
        'cycle_metrics': [first_cycle_metrics]  # 初始化循环指标列表
        })
    
        # 初始化总体统计
        self.overall_min_distance = distance
        self.overall_max_distance = distance
        ...
        - Starting Cycle: 1 of {self.max_protocol_cycles}
    ...
    def complete_single_cycle(self):
        current_time = self.carla.world.get_snapshot().timestamp.elapsed_seconds
        self.current_cycle += 1
    
        # 记录当前循环的结束时间和指标
        current_cycle_metrics = self.cycle_template.copy()
        current_cycle_metrics.update({
        'cycle_number': self.current_cycle,
        'start_time': self.phase_start_time if hasattr(self, 'phase_start_time') else current_time,
        'end_time': current_time,
        'min_distance': self.challenge_metrics['min_distance'],
        'max_distance': self.challenge_metrics['max_distance'],
        'maintain_start_time': self.challenge_metrics.get('maintain_start_time'),
        'retreat_start_time': self.challenge_metrics.get('retreat_start_time'),
        'new_maintain_start_time': self.challenge_metrics.get('new_maintain_start_time')
        })
    
        self.cycle_metrics.append(current_cycle_metrics)
        self.challenge_metrics['cycle_metrics'].append(current_cycle_metrics)
        self.plot_data['cycle_boundaries'].append(current_time)
    
        if self.current_cycle >= self.max_protocol_cycles:
            ...
            # 重置当前循环的距离指标
            self.challenge_metrics['min_distance'] = float('inf')
            self.challenge_metrics['max_distance'] = 0
        
            # 为新循环初始化指标
            new_cycle_metrics = self.cycle_template.copy()
            new_cycle_metrics.update({
            'cycle_number': self.current_cycle + 1,
            'start_time': current_time
            })

            logging.info(f"""
            Cycle {self.current_cycle} completed, starting cycle {self.current_cycle + 1}:
            - Time: {current_time:.2f}s
            - Starting Phase: APPROACH
            - Target Distance: {self.target_distance}m
            """)

    def _format_cycle_statistics(self):
        stats = ""
        for cycle in self.cycle_metrics:
            duration = cycle['end_time'] - cycle['start_time'] if cycle['end_time'] and cycle['start_time'] else 0
            stats += f"""
            Cycle {cycle['cycle_number']}:
            - Duration: {duration:.2f}s
            - Distance Range: {cycle['min_distance']:.2f}m to {cycle['max_distance']:.2f}m
            - Variation: {cycle['max_distance'] - cycle['min_distance']:.2f}m
            """
        return stats
    ...
    def complete_challenge(self):
        ...
        # 计算所有循环的综合统计信息
        total_duration = current_time - self.challenge_metrics['activation_time']
    
        logging.info(f"""
        Physical Challenge Protocol Fully Completed:
        - Total Cycles Completed: {self.current_cycle}
        - Total Duration: {total_duration:.2f}s
    
        Cycle Statistics:
        {self._format_cycle_statistics()}
        """)
    ...
    def update_protocol_phase(self, current_time, distance):
        ...
        if self.current_phase == self.protocol_phases['APPROACH']:
            if elapsed_time >= 3 and abs(distance - self.target_distance) < self.distance_threshold:
    ...
    def plot_results(self):
        ...
        gs = GridSpec(5, 1, figure=fig, height_ratios=[3, 2, 2, 2, 0.8])
        ...
        completed_color = 'wheat'

        # 获取初始和结束时间
        start_time = min(self.plot_data['timestamps'])
        end_time = max(self.plot_data['timestamps'])

        # 绘制INACTIVE背景
        ax5.axvspan(start_time, end_time, ymin=0.1, ymax=0.9, color=inactive_color, alpha=0.5)

        # 如果有循环边界，绘制循环区块
        if self.plot_data['cycle_boundaries']:
            # 添加协议开始时间
            protocol_start = self.challenge_metrics.get('start_timestamp', start_time)
        
            # 绘制每个循环的区块
            cycle_times = [protocol_start] + self.plot_data['cycle_boundaries']
            for i in range(len(cycle_times)-1):
                cycle_start = cycle_times[i]
                cycle_end = cycle_times[i+1]
            
                # 绘制循环区块
                ax5.axvspan(cycle_start, cycle_end, ymin=0.1, ymax=0.9, color=active_color, alpha=0.5)
            
                # 添加循环标签
                mid_point = (cycle_start + cycle_end) / 2
                ax5.text(mid_point, 0.5, f'Cycle {i+1}', ha='center', va='center', bbox=dict(facecolor='white', alpha=0.7, pad=2))

            # 如果协议已完成，标记完成部分
            if self.protocol_completed:
                ax5.axvspan(cycle_times[-1], end_time, ymin=0.1, ymax=0.9, color=completed_color, alpha=0.5)

        # 设置轴标签等
        ax5.set_yticks([])
        ax5.set_xlabel('Time (s)')
        ax5.set_title('Protocol Cycles')
        ...
        if hasattr(self, 'challenge_metrics'):
            ...
            if start_time is not None and end_time is not None:
                metrics_text = (f"Protocol Metrics:\n"
                            f"Total Duration: {end_time - start_time:.1f}s\n"
                            f"Initial Distance: {self.challenge_metrics.get('initial_distance', 0):.1f}m")
                plt.figtext(0.75, 0.02, metrics_text, fontsize=10, 
