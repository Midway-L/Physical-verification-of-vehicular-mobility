...
class SimulationSynchronization(object):
    def __init__(self, sumo_simulation, carla_simulation, tls_manager='none', sync_vehicle_color=False, sync_vehicle_lights=False):
        ...
        self.challenge_duration = 40  # seconds in total challenge, add all phase durations
    ...
    def complete_challenge(self):
        ...
        if approach_time != 'N/A':
            approach_time = f"{approach_time - self.challenge_metrics['activation_time']:.2f}"
    
        maintain_time = self.challenge_metrics.get('maintain_start_time', 'N/A')
        if maintain_time != 'N/A':
            maintain_time = f"{maintain_time - self.challenge_metrics['activation_time']:.2f}"
        ...
        f"- Approach Phase Complete Time: {approach_time}s\n"
        f"- Maintain Phase Start Time: {maintain_time}s\n"
        f"- Initial Distance: {self.challenge_metrics['initial_distance']:.2f}m\n"
        f"- Minimum Distance: {self.challenge_metrics['min_distance']:.2f}m\n"
        f"- Maximum Distance: {self.challenge_metrics['max_distance']:.2f}m\n"
        f"- Distance Variation: {distance_variation:.2f}m"
        )
    ...
    def update_protocol_phase(self, current_time, distance):
        ...
        logging.info("Current phase: %s, Distance: %.2fm, Target: %.2fm, Elapsed time in phase: %.2fs", self.current_phase, distance, self.target_distance, elapsed_time)
    ...
    def control_following_vehicle(self):
        ...
        if self.challenge_active:
            ...
            print(f"- Krauss Safe Speed: {safe_speed:.2f} m/s" f"- Target Speed: {target_speed:.2f} m/s" f"- Final Speed: {final_speed:.2f} m/s")
    ...
    def plot_results(self):
        ...
        # 添加目标距离参考线
        for phase in ['APPROACH', 'MAINTAIN']:
            if phase in self.phase_distances:
                target_dist = self.phase_distances[phase]
                ax1.axhline(y=target_dist, color='g', linestyle='--', alpha=0.3, label=f'{phase} Target ({target_dist}m)')
        ...
        ax2.plot(self.plot_data['timestamps'], angle_errors, 'g-', label='Angle Error', linewidth=2)
        ax2.axhline(y=0, color='k', linestyle='--', alpha=0.5)
        ax2.set_xlabel('Time (s)')
        ax2.set_ylabel('Angle Error (rad)')
        ax2.legend()
        ax2.grid(True)
        ax2.set_title('Relative Angle Error')
        ...
        distance_errors = np.array(self.plot_data['true_distances']) - np.array(target_distances)
        ...
        # 绘制阶段时间轴
        current_phase = self.plot_data['phases'][0]
        phase_start = self.plot_data['timestamps'][0]
        y_position = 0

        for i, (time, phase) in enumerate(zip(self.plot_data['timestamps'], self.plot_data['phases'])):
            if phase != current_phase or i == len(self.plot_data['timestamps']) - 1:
                # 绘制阶段块
                ax5.axvspan(phase_start, time, ymin=0.1, ymax=0.9, 
                       color=phase_colors.get(current_phase, 'white'), alpha=0.5)
                # 添加阶段标签
                mid_point = (phase_start + time) / 2
                ax5.text(mid_point, y_position + 0.5, current_phase, 
                    ha='center', va='center', rotation=0)
            
                # 在其他图表中添加垂直线标记阶段变化
                if current_phase != 'INACTIVE':
                    for ax in [ax1, ax2, ax3, ax4]:
                        ax.axvline(x=time, color='g', linestyle='--', alpha=0.3)
