...
class SimulationSynchronization(object):
    ...
    def __init__(self,
        ...
        #用于生成正弦波的参数
        self.base_speed = 12.5  # m/s, 基础速度
        self.speed_amplitude = 1.4  # m/s, 速度变化幅度
        self.speed_period = 10  # s, 速度变化周期
        self.target_distance = 5 # m, 目标跟车距离 sumo and carla have about 5-6 meters gap 
    ...
    def control_following_vehicle(self):
        ...
        #traci.vehicle.setMinGap(self.following_vehicle_id, 0)  # 允许更接近的跟随
        ...
        if leader:
            ...
            # 计算前馈控制项
            feedforward = self.base_speed + self.speed_amplitude * np.sin(2 * np.pi * (current_time + 0.5) / self.speed_period)
            
            # PID 控制
            distance_error = distance - self.target_distance
            self.error_integral += distance_error * self.carla.step_length
            speed_diff = lead_speed - current_speed
            
            p_term = self.kp * distance_error
            i_term = self.ki * self.error_integral
            d_term = self.kd * speed_diff
            
            # 合并反馈控制和前馈控制
            acceleration = p_term + i_term + d_term + (feedforward - current_speed) / self.carla.step_length
            
            # 限制加速度
            max_accel = 2.6
            max_decel = 4.5
            acceleration = max(-max_decel, min(max_accel, acceleration))
            
            new_speed = current_speed + acceleration * self.carla.step_length
            new_speed = max(0, new_speed)  # 确保速度非负
            
            traci.vehicle.setSpeed(self.following_vehicle_id, new_speed) 
            
            print(f"Following - Distance: {distance:.2f}m, Speed: {current_speed:.2f} -> {new_speed:.2f} m/s")

            #print(f"Following - Distance: {distance:.2f}m, Speed: {current_speed:.2f}m/s")
            
             # 自适应控制：根据误差调整增益
            if abs(distance_error) > 0.5:  # 如果误差较大
                self.kp *= 1.05  # 增加比例增益
                self.kd *= 1.05  # 增加微分增益
            else:
                self.kp *= 0.95  # 减小比例增益
            ...
            print("No leader found, maintaining speed")
