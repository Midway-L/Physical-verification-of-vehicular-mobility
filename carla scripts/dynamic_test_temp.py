...
class SimulationSynchronization(object):
    ...
    def initialize_sensors_and_communication(self):
        ...
        # 创建新的EKF实例
        init_x = np.array([0, 0, 0])  # 初始状态 [rx, ry, theta]
        init_P = np.diag([100, 10, np.pi/2])  # 初始协方差
        self.ekf = SimpleEKF(dt=0.1, init_x=init_x, init_P=init_P)
        self.distance_bias = 2.4
    ...
    def calculate_safe_speed(self, gap, speed, lead_speed): # adapted from Krauss follow speed calculation
        ...
        if gap is None or speed is None or lead_speed is None:
            return speed
    ...
    def calculate_normal_following_speed(self, distance, current_speed, lead_speed, current_time):
        ...
        if distance is None:
            return current_speed  # 返回当前速度作为安全值
    ...
    def get_carla_distance(self, vehicle1_id, vehicle2_id):

        # Get CARLA actor IDs from SUMO IDs
        carla_id1 = self.sumo2carla_ids.get(vehicle1_id)
        carla_id2 = self.sumo2carla_ids.get(vehicle2_id)
    
        if carla_id1 is None or carla_id2 is None:
            return None
        
        # Get CARLA actors
        vehicle1 = self.carla.world.get_actor(carla_id1)
        vehicle2 = self.carla.world.get_actor(carla_id2)
        ...
        # Calculate 2D Euclidean distance (only x and y coordinates)
    ...
    def control_following_vehicle(self):
        ...
        # 基本参数获取
        distance = self.get_carla_distance(self.following_vehicle_id, self.lead_vehicle_id)
        if distance is None:
            distance = sumo_distance + 4.87 # 补偿距离
        ...
        # Update plot data  
        r = np.sqrt(self.ekf.x[0]**2 + self.ekf.x[1]**2)# 使用x,y计算估计距离
        distance_estimate = r + self.distance_bias
    ...
    def plot_results(self):
        ...
        for i, phase in enumerate(self.plot_data['phases']):
            if phase != 'INACTIVE':
                active_start_idx = i
                break
        ...
        if active_start_idx is not None:
            # 只绘制协议激活后的数据
            active_timestamps = self.plot_data['timestamps'][active_start_idx:]
            active_estimated_distances = self.plot_data['estimated_distances'][active_start_idx:]
            active_true_distances = self.plot_data['true_distances'][active_start_idx:]

            ax1.plot(active_timestamps, active_estimated_distances, 
                    'b-', label='Estimated Distance', linewidth=2)
            ax1.plot(active_timestamps, active_true_distances, 
                    'r-', label='True Distance', linewidth=2)

            # 计算统计数据
            dist_errors = np.array(active_estimated_distances) - np.array(active_true_distances)
            ...
            ax1.text(0.98, 0.02, dist_stats, transform=ax1.transAxes, 
        ...
        for phase, color in phase_colors.items():
            if phase in self.phase_distances:
                ...
                ax1.axhline(y=target_dist, color=color, linestyle='--', alpha=0.3, 
                            label=f'{phase} Distance ({target_dist}m)')
        ...
        if active_start_idx:
            angle_errors = np.array(self.plot_data['estimated_angles'])[active_start_idx] - \
                   np.array(self.plot_data['true_angles'])[active_start_idx]
        ...
        error_threshold = 2.0  # 2米的阈值

        if active_start_idx is not None:
            # 只处理协议激活后的数据
            active_timestamps = self.plot_data['timestamps'][active_start_idx:]
            active_estimated_distances = self.plot_data['estimated_distances'][active_start_idx:]
            active_phases = self.plot_data['phases'][active_start_idx:]

            # 计算目标距离和误差
            target_distances = [self.phase_distances.get(phase, 10) for phase in active_phases]
            distance_errors = np.array(active_estimated_distances) - np.array(target_distances)

            # 绘制基础误差线
            ax3.plot(active_timestamps, distance_errors, 'r-', label='Distance Error', linewidth=2)

            # 添加阈值参考线
            ax3.axhline(y=error_threshold, color='r', linestyle='--', alpha=0.5, 
                        label=f'2m Threshold (±{error_threshold}m)')
            ax3.axhline(y=-error_threshold, color='r', linestyle='--', alpha=0.5)
            ...
            # 计算误差超过阈值的时间百分比
            # error_time = sum(1 for error in distance_errors if abs(error) > error_threshold)
            # total_time = len(distance_errors)
            # error_percentage = (error_time / total_time) * 100 if total_time > 0 else 0

            target_stats = (
                   f'RMSE: {target_rmse:.2f}m\n'
                   f'MAE: {target_mae:.2f}m\n'
                   f'Max Error: {target_max_error:.2f}m\n')
                   #f'Time Above Threshold: {error_percentage:.1f}%')
    
            ax3.text(0.98, 0.02, target_stats, transform=ax3.transAxes, 
                    bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'),
                    verticalalignment='bottom', horizontalalignment='right', fontsize=10)
